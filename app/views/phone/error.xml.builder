...
xml.Response do 
  ...
  if @error == "number_problem_1"
    xml.Sms("The system does not recognize this number. Please send texts to Hdmon from the phone number associated with your account.")
   
  elsif @error == "number_problem_2"
    xml.Sms("There is a problem with the phone number you are sending texts from. Please send texts to Hdmon from the phone number associated with your account.")

  elsif @error == "unrecognized_response_1"
    xml.Sms("Unrecognized response. Text 'yes' if you had a headache today. Text 'no' if you did NOT have headache today.")
  
  elsif @error == "unrecognized_response_2"
       xml.Sms("There was a problem with your response. Please respond by texting 'M' for mild or 'S' for severe to describe your headache today.")
